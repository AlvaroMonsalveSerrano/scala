...
//  import cats.instances.all._
  ...
  def ejemplo1(): Unit = {
    ...
    val strs = "!"
    println(
      s" 123.invalid[String].bimap( _ + '!', _ * 100 ) = ${123.invalid[String].bimap(elem => s"$elem$strs", _ * 100)}"
    )
    println()
  ...
  def ejemplo2(): Unit = {
    ...
    println(
      s"41.valid[String].withEither( _.flatMap( n => Right(n + 1)) )=${41.valid[String].withEither(_.flatMap(n => Right(n + 1)))}"
    )
    println()
