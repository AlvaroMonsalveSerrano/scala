...
trait PrintableContramapInstances {
  ...
  implicit def boxPrintable[A](implicit P: PrintableContramap[A]): PrintableContramap[Box[A]] =
    new PrintableContramap[Box[A]] { self =>
...
trait PrintableContramapSyntax {
  object syntax {
    ...
    def B_A[B, A](f: B => A)(implicit P: PrintableContramap[A]): PrintableContramap[B] =
      P.contramap(f) // OJO CON LOS TIPOS EN UNA ES A Y EN OTRO B
  }
}
