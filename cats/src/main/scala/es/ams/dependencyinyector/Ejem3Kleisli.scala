...
//      import cats.FlatMap
      ...
      def clientOrder[M[_]](implicit trading:Trading3Generic[M]):  Kleisli[M, ClientOrder, Order] = trading.clientOrder()
      def execute[M[_]](m: Market, a: Account)(implicit trading:Trading3Generic[M]): Kleisli[M, Order, Execution] = trading.execute(m,a)
      def allocate[M[_]](as: List[Account])(implicit trading:Trading3Generic[M]):Kleisli[M, Execution, Trade] = trading.allocate(as)
