...
object Ejem1Foldable extends App {
  ...
  def ejemploFilter(): Unit = {
    ...
    println(
      s"List(1, 2, 3, 4) existe el 3?=${List(1, 2, 3, 4).foldRight(false)((elem, resul) => resul || elem.equals(elemFilter1))}"
    )

    val elemFilter2: Int = 5
    println(
      s"List(1, 2, 3, 4) existe el 5?=${List(1, 2, 3, 4).foldRight(false)((elem, resul) => resul || elem.equals(elemFilter2))}"
    )
    println()
  ...
  def ejemploMap(): Unit = {
    def myMap[A, B](list: List[A])(f: A => B): List[B] =
      list.foldRight(List.empty[B])((elem, result) => f(elem) :: result)

    println(
      s"List(1, 2, 3) map to String=${List(1, 2, 3).foldRight(List.empty[String])((elem, resul) => s"-${elem.toString}-" :: resul)}"
    )
