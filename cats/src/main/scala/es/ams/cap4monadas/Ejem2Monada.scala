...
object Ejem2Monada extends App {
  ...
  def sumaMonadica[F[_]: Monad](e1: F[Int], e2: F[Int], e3: F[Int]): F[Int] = {
    e1.flatMap(a => e2.flatMap(b => e3.map(c => a + b + c)))
  }
  ...
  def monadaFlatMapFor(): Unit = {
    ...
    println(
      s"sumaMonadica(Some(2), Some(3), Some(4))=${sumaMonadica(monadaOption.pure(2), monadaOption.pure(3), monadaOption.pure(4))}"
    )
    println()
    println(
      s"sumaMonadicaFor(Some(2), Some(3), Some(4))=${sumaMonadicaFor(monadaOption.pure(2), monadaOption.pure(3), monadaOption.pure(4))}"
    )
    println()
